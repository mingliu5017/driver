	...
	if (coeff >= 0) {
		for (index = 0; index < (ES7243E_CHANNELS_MAX) / 2; index++) {
			...
			es7243e_write(ES7243E_PREDIV,
				      0x23,//coeff_div[coeff].prediv_premulti,
				      i2c_clt[index]);
...
static void es7243e_parse_dt(struct device_node *np, struct es7243e_priv *es7243e)
{
    int ret = 0;
    int workmode = ES7243E_NORMAL_I2S;
    int mclkinv = MCLK_NORMAL;
    int bclkinv = BCLK_NORMAL;

    ret = of_property_read_u32(np, "workmode", &workmode);
    if (ret) {
        pr_err("%s %d no 'workmode' node, use default: ES7243E_NORMAL_I2S\n", \
               __func__, __LINE__);
        es7243e->tdm = ES7243E_NORMAL_I2S;
    } else {
        es7243e->tdm = workmode;
    }


    es7243e->tdm &= 0xfc;
	es7243e->tdm |= 0x01;  //watson 

    pr_info("%s %d es7243e->tdm:%d\n", __func__, __LINE__, es7243e->tdm);
    
    ret = of_property_read_u32(np, "mclkinv", &mclkinv);
    if (ret) {
        pr_err("%s %d no 'mclkinv' node, use default: MCLK_NORMAL\n", \
               __func__, __LINE__);
        es7243e->mclkinv = MCLK_NORMAL;
    } else {
        es7243e->mclkinv = mclkinv;
    }
    pr_info("%s %d es7243e->mclkinv:%d\n", __func__, __LINE__, es7243e->mclkinv);
    
    ret = of_property_read_u32(np, "bclkinv", &bclkinv);
    if (ret) {
        pr_err("%s %d no 'bclkinv' node, use default: BCLK_NORMAL\n", \
               __func__, __LINE__);
        es7243e->bclkinv = BCLK_NORMAL;
    } else {
        es7243e->bclkinv = bclkinv;
    }
	
	es7243e->bclkinv = 0x00; //watson 
    pr_info("%s %d es7243e->bclkinv:%d\n", __func__, __LINE__, es7243e->bclkinv);
}
...
es7243e_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *i2c_id)
{
    ...
    es7243e_parse_dt(i2c->dev.of_node, es7243e);
